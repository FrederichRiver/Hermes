...
class MySQLAdmin(MySQLMeta):
    """Administrator role: schema and permissions management helpers."""
    ...
    def grant_privileges(self, user: str, host: str, privileges: str, db: str, table: str = '*'):
        sql = f"GRANT {privileges} ON `{db}`.`{table}` TO %s@%s"
        return self.execute(sql, (user, host))

    def revoke_privileges(self, user: str, host: str, db: str, table: str = '*'):
        sql = f"REVOKE ALL PRIVILEGES ON `{db}`.`{table}` FROM %s@%s"
        return self.execute(sql, (user, host))


class StockEngine(MySQLMeta):
    """Limited-role DB access for stock-related operations.

    - Exposes CRUD methods for a `stocks` table (id, code, name, index, extra JSON).
    - Transactional multi-row operations should be orchestrated by caller (e.g. crawler).
    """
    ...
    def insert_stock(self, code: str, name: str = None, idx: typing.Optional[int] = None, extra: typing.Optional[str] = None) -> int:
        sql = "INSERT INTO `stocks` (`code`, `name`, `idx`, `extra`) VALUES (%s, %s, %s, %s)"
        return self.execute(sql, (code, name, idx, extra))

    def update_stock(self, code: str, **fields) -> int:
        if not fields:
            return 0
        cols = ', '.join(f"`{k}` = %s" for k in fields.keys())
        params = tuple(fields.values()) + (code,)
        sql = f"UPDATE `stocks` SET {cols} WHERE `code` = %s"
        return self.execute(sql, params)

    def delete_stock(self, code: str) -> int:
        sql = "DELETE FROM `stocks` WHERE `code` = %s"
        return self.execute(sql, (code,))

    def get_stock(self, code: str) -> typing.Optional[dict]:
        rows = self.query("SELECT * FROM `stocks` WHERE `code` = %s", (code,))
        return rows[0] if rows else None

    def list_stocks(self, limit: int = 100, offset: int = 0) -> typing.List[dict]:
        return self.query("SELECT * FROM `stocks` ORDER BY `id` DESC LIMIT %s OFFSET %s", (limit, offset))
